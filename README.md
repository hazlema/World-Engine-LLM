...
- **Claude-family archivist routing unblocked.** Pointing the archivist at a Claude model (e.g. `anthropic/claude-haiku` via OpenRouter) used to fail with `OpenRouter 400` immediately — Amazon Bedrock's strict JSON-schema validator rejects `maxItems` on array properties and `minimum` on integer items, both of which the archivist schema was declaring. Caps and range checks were already enforced server-side after the model returned (slice + safety net + integer filter), so dropping the schema-side declarations changes nothing about behavior but lets Claude-routed archivists actually work. Opens up the same-family routing experiment (narrator and archivist both on Claude) for testers comparing it against the previous mixed-family default.

- **Debug panel image-style label now matches reality.** The Providers row always read `image: gemini / cinematic` regardless of which style you'd actually picked, because image style is client-side state (in localStorage) but the panel was rendering the server-side default. Now reads the client's live `imageStyle`, so the line matches what's actually being used.
