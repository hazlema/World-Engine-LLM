...
- When an active objective names a specific item AND that item is in ESTABLISHED WORLD (e.g. active "Find the transmitter" + entry "damaged transmitter half-buried in regolith"), that item IS at the current tile. The narrative must reference the named item by its established name (or an unambiguous descriptor of it) when the player arrives, looks around, or examines. Do not substitute a different object (a torn sheet of aluminum, a stray cable, an empty container) to delay the find. Mystery and intrigue around the item are fine — claw marks on the transmitter, a bent antenna, a strange hum — but the item itself must be findable, not replaced.
