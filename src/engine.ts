...
- When an active objective names a specific item AND that item is in ESTABLISHED WORLD (e.g. active "Find the transmitter" + entry "damaged transmitter half-buried in regolith"), that item IS at the current tile. The narrative must reference the named item by its established name (or an unambiguous descriptor of it) when the player arrives, looks around, or examines. Do not substitute a different object (a torn sheet of aluminum, a stray cable, an empty container) to delay the find. Equally, do not preemptively deny presence ("no transmitter here", "no sign of the transmitter", "nothing resembling a transmitter") — denial is just substitution-by-negation. The item IS at this tile; describe it as present, with whatever damage / mystery / atmospheric details fit. Mystery and intrigue around the item are fine — claw marks on the transmitter, a bent antenna, a strange hum — but the item itself must be findable, not replaced and not denied.
- If an item or piece of equipment is in ESTABLISHED WORLD (especially preset-seeded items like "damaged transmitter half-buried in regolith"), use its name when the narrative surfaces or references it — whether the player is at the item's tile or seeing it from a distance. Atmospheric descriptors ("a metallic glint catches the light", "a dark shape against the rocks") are fine ALONGSIDE the canonical noun ("a metallic glint catches the light from the damaged transmitter, half-buried in the regolith to the north"), but do not surface established items via vague descriptors alone. The player must be able to reference items back by name; if the narrative never names a thing, the player cannot interact with it.
...
GOOD endings (one concrete beat, no question, no menu):
- "A loose bolt rattles across the dust and stops at your boots."
- "The lander's beacon flashes once, then stays steady."
- "Footprints — not yours — lead away from the rover, fresh in the dust."
- "Distant: the rhythmic ping again, slower than before."
- "A faint glint reflects off the rover's chassis in the distance."
- "A discarded geological hammer lies near the rover wreck."
...
- For LOCATE objectives — and ONLY objectives whose verb is "Find", "Locate", or "Reach" (or close synonyms like "Discover the location of"). The player arriving at the objective's coordinate AND the NEW NARRATIVE this turn naming or depicting the target satisfies the objective — RETURN THE INDEX. The position gate already filters out distant tiles via the [DISTANT — cannot be completed this turn] suffix; when the player is at the target tile AND the narrative names the target, that's a find. **IMPORTANT EXCEPTION:** the "observation, approach, or atmospheric clue is NOT completion" rule above DOES NOT APPLY to LOCATE objectives — for LOCATE, observation at the target tile IS the completion event. Locating something IS observing it.
- LOCATE does NOT apply to action verbs. Objectives starting with "Send" ("Send the distress signal"), "Restore" ("Restore power to the comm array"), "Repair" / "Fix" / "Light" / "Open" / "Break" / "Activate" / "Use" / "Place" / "Take" / any other action verb are PHYSICAL ACTION objectives and require depicted state change (the rule above), NOT mere arrival. Do not fire a "Send the distress signal" objective just because the player walked to the transmitter's tile — they must actually use the transmitter to send something, and the narrative must depict that. Same for "Restore power": being at the comm array is not enough; the narrative must depict the power coming back.
- Examples that DO complete "Find the transmitter": "you crest the regolith ridge; the damaged transmitter half-buried in dust comes into view", "your boots find purchase at the transmitter site, the antenna mast leaning askew", "the transmitter is here, sunken under decades of dust".
- Examples that DO NOT complete "Find the transmitter": "you spot the transmitter on the far rim" (still on a different tile — would be filtered by [DISTANT] anyway, but be explicit), "the transmitter must be somewhere ahead" (no arrival depicted), "you recall the transmitter from the briefing" (not from THIS turn's narrative).
