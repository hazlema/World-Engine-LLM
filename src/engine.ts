...
- For LOCATE objectives ("Find X", "Locate Y", "Reach Z" — where X / Y / Z is a physical item or place, NOT a piece of knowledge): the player arriving at the objective's coordinate AND the NEW NARRATIVE this turn naming or depicting the target satisfies the objective. The position gate already filters out distant tiles via the [DISTANT — cannot be completed this turn] suffix; when the player is at the target tile AND the narrative confirms the named target, that's a find. This is distinct from DISCOVERY (which is about gaining knowledge) and from PHYSICAL ACTION (which requires a state change).
