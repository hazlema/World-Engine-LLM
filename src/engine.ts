...
- Do NOT use emdashes! Use a comma instead.
...
- For LOCATE objectives — and ONLY objectives whose verb is "Find", "Locate", or "Reach" (or close synonyms like "Discover the location of"). The player must take an EXPLICIT OBSERVATION action this turn (look, examine, inspect, search, study, scan, observe, read, peer, gaze) AT the objective's coordinate, AND the new narrative this turn must name or depict the target. BOTH halves required — RETURN THE INDEX only if both are present. Mere arrival at the target tile (e.g. a cardinal move like "north" or "go east") does NOT count, even if the narrator names the target on arrival — the player must actively look. The position gate filters off-tile objectives via the [OFF-TILE — cannot be completed this turn] suffix. **IMPORTANT EXCEPTION:** the "observation, approach, or atmospheric clue is NOT completion" rule above DOES NOT APPLY to LOCATE objectives — for LOCATE, observation at the target tile IS the completion event. Locating something IS observing it.
- LOCATE does NOT apply to action verbs. Objectives starting with "Send", "Restore", "Repair", "Fix", "Light", "Open", "Break", "Activate", "Use", "Place", "Take", or any other action verb are PHYSICAL ACTION objectives and require depicted state change (the rule above), NOT mere arrival or observation. Do not fire an action-verb objective just because the player walked to the target item's tile or looked at it — they must actually perform the action, and the narrative must depict that.
- Examples that DO complete "Find the iron key" (player took an observation action at the key's tile AND the narrative names it): player input "look around" with narrative "your eyes sweep the room; the iron key rests on the chest in plain sight", player input "examine the desk" with narrative "you crouch and inspect the table — the iron key glints among the scrap", player input "search the floor" with narrative "you scan the boards and the iron key surfaces, half-buried in dust". (Substitute whatever item the actual objective and entry name — the principle is: observation action + canonical item named.)
- Examples that DO NOT complete "Find the iron key": player input "north" with narrative "you cross the threshold; the iron key catches the light" (arrived, did not actively look — the player must explicitly observe), player input "go east" with narrative "your boots find purchase at the chest, the iron key here" (same — arrival is not observation), "you spot the iron key on the far shelf" (still on a different tile — would be filtered by [OFF-TILE] anyway), "the iron key must be somewhere ahead" (no arrival depicted), "you recall the iron key from the briefing" (not from THIS turn's narrative).
