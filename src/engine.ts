...
- For LOCATE objectives ("Find X", "Locate Y", "Reach Z" — where X / Y / Z is a physical item or place, NOT a piece of knowledge): the player arriving at the objective's coordinate AND the NEW NARRATIVE this turn naming or depicting the target satisfies the objective — RETURN THE INDEX. The position gate already filters out distant tiles via the [DISTANT — cannot be completed this turn] suffix; when the player is at the target tile AND the narrative names the target, that's a find. **IMPORTANT EXCEPTION:** the "observation, approach, or atmospheric clue is NOT completion" rule above DOES NOT APPLY to LOCATE objectives — for LOCATE, observation at the target tile IS the completion event. Locating something IS observing it. Do not withhold the completion because the narrative reads as "observation". This is distinct from DISCOVERY (which is about gaining knowledge) and from PHYSICAL ACTION (which requires a state change).
