...
- When an active objective names a specific item AND that item is in ESTABLISHED WORLD (e.g. active "Find the ITEM-X" + entry "ITEM-X (with established detail) here"), that item IS at the current tile. The narrative must reference the named item by its established name (or an unambiguous descriptor of it) when the player arrives, looks around, or examines. Do not substitute a different object (a similar-looking thing, a partial piece, an empty container) to delay the find. Equally, do not preemptively deny presence ("no ITEM-X here", "no sign of ITEM-X", "nothing resembling ITEM-X") — denial is just substitution-by-negation. The item IS at this tile; describe it as present, with whatever damage / mystery / atmospheric details fit. Mystery and intrigue around the item are fine, but the item itself must be findable, not replaced and not denied.
- If an item or piece of equipment is in ESTABLISHED WORLD (especially preset-seeded items with specific names in the entries list), use its name when the narrative surfaces or references it — whether the player is at the item's tile or seeing it from a distance. Atmospheric descriptors are fine ALONGSIDE the canonical noun, but do not surface established items via vague descriptors alone. The player must be able to reference items back by name; if the narrative never names a thing, the player cannot interact with it.
...
- The OBJECTIVES list (if present) is shown with its indices: "0: [ ] Find the ITEM-X".
...
- Examples that DO complete "Find the ITEM-X" when the player arrives at ITEM-X's tile and the new narrative names it: "you crest the rise; ITEM-X comes into view", "your boots find purchase at the site, ITEM-X here in plain sight", "ITEM-X is here, beneath the dust".
- Examples that DO NOT complete "Find the ITEM-X": "you spot ITEM-X on the far rim" (still on a different tile — would be filtered by [OFF-TILE] anyway, but be explicit), "ITEM-X must be somewhere ahead" (no arrival depicted), "you recall ITEM-X from the briefing" (not from THIS turn's narrative).
