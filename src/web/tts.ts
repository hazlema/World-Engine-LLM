...
export class TTSEngine {
  ...
  render(turnId: number, text: string, voice?: string, lengthScale?: number): Promise<RenderResult> {
    return this.queue.enqueue(async () => {
      ...
      const res = await fetch("/api/speak", {
        ...
        body: JSON.stringify({ text, voice, lengthScale }),
      });
      if (!res.ok) {
      ...
      console.info(`[tts] render turn ${turnId}: ${text.length} chars in ${Math.round(dur)}ms (${voice ?? "default"} ×${lengthScale ?? 1})`);
